...
class CTkDatePicker(ctk.CTkFrame):
    def __init__(self, master, app_logic: AppLogic, year: int, tab_month: int, day: int, allow_month_change: bool = True, allow_manual_input: bool = True, **kwargs):
        """
        ...
        self.allow_change_month = allow_month_change
        self.add_months = 0
    ...
    def open_calendar(self):
        ...
        self.popup.after(500, lambda: self.set_toplevel_focus(self.popup))        

        self.build_calendar()
    
    def set_toplevel_focus(self, top_level: ctk.CTkToplevel | None):
        if top_level == None:
            pass
        else:
            top_level.focus()
    ...
    def build_calendar(self):
        ...
        month_label = ctk.CTkLabel(self.calendar_frame, text=f"{calendar.month_name[self.month].capitalize()}, {self.year}")
    ...
    def select_date(self, day: int):
        ...
        if self.month == 0 or day == 0:
            self.selected_date_str: str = (f'{0}/{0}/{self.year}')
            self.date_entry.insert(0, self.selected_date_str)
        else:
    ...
    def set_popup_bindings(self):
        if self.popup is not None:
            self.popup.bind("<FocusOut>", self.destroy_popup_on_click_outside)
    
    def destroy_popup_on_click_outside(self, event):
        if self.popup is not None:
            self.popup.destroy()