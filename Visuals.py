...
class VisualFunctions(ctk.CTkBaseClass):
    ...
    def configure_widget(self, widget: ctk.CTkBaseClass, new_text:str = ""):
    ...
    def get_hierarchy_item(self, hierarchy_name: ttk.Treeview, item_name: str | int, new_text: str | None=None, new_values: list | None=None,  display_open: bool=False):
        '''NOTE: returns a treeview item dict, and allows editing of existing item's content'''
    ...
    def get_hierarchy_item_children(self, hierarchy_name: ttk.Treeview, item_name: str | int) -> tuple[str, ...]:
    ...
    #textbox: .get .delete .markset .insert .focus, focus_set
    ...
    #NOTE: define these if needed later
    def get_textbox_focus(self):
        pass
...
class MainMenu(ctk.CTkFrame):
    def __init__(self, master: Any, create_new_system: Enum, manage_budget_system: Enum, app_logic: AppLogic):
        super().__init__(master)
...
class RadioButtonMenu(ctk.CTkFrame):
    def __init__(self, parent, system_name: Enum, app_logic: AppLogic, menu_title: str):
    ...
    def extract_selected_template_string_var(self):
        self.selected_template_name_str: str = self.selected_template_name_widget_str.get()
        return self.selected_template_name_str
...
class EditBudgetTemplate(ctk.CTkFrame):
    def __init__(self, parent, system_name: Enum, app_logic: AppLogic):
        super().__init__(master=parent)
        ...
        self.category_text_box.bind("<FocusIn>", lambda event: app_logic.add_context_menu_textbox_focus(event, self.category_text_box))
        self.category_text_box.bind("<FocusOut>", lambda event: app_logic.remove_context_menu_textbox_focus(event, self.category_text_box))

        # #category focus
        self.category_text_box_in_category_menu.bind("<FocusIn>", lambda event: app_logic.add_context_menu_textbox_focus(event, self.category_text_box_in_category_menu))
        self.category_text_box_in_category_menu.bind("<FocusOut>", lambda event: app_logic.remove_context_menu_textbox_focus(event, self.category_text_box_in_category_menu))

        self.subcategory_text_box_in_category_menu.bind("<FocusIn>", lambda event: app_logic.add_context_menu_textbox_focus(event, self.subcategory_text_box_in_category_menu))
        self.subcategory_text_box_in_category_menu.bind("<FocusOut>", lambda event: app_logic.remove_context_menu_textbox_focus(event, self.subcategory_text_box_in_category_menu))

        #subcategory focus 
        self.subcategory_textbox.bind("<FocusIn>", lambda event: app_logic.add_context_menu_textbox_focus(event, self.subcategory_textbox))
        self.subcategory_textbox.bind("<FocusOut>", lambda event: app_logic.remove_context_menu_textbox_focus(event, self.subcategory_textbox))

        app_logic.add_to_nav_map(system_name.value, self)

    def extract_int_var(self):
        self.monthly_annual_int: list[int] = [self.monthly_annual.get()]
        return self.monthly_annual_int
...
class Comp2pg2(ctk.CTkFrame):
    def __init__(self, master: Any, system_name: Enum, app_logic: AppLogic):
        super().__init__(master)
